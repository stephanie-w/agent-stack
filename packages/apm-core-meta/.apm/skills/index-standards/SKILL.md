...
1. List all `.md` files in `standards/` and its subfolders
...
Read `standards/index.yml` if it exists. Note which entries already have descriptions.
...
Generate `standards/index.yml` with this structure:
...
**Note:** `root` appears first and contains standards files that live directly in `standards/` (not in subfolders).
