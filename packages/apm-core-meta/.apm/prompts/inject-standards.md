...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `standards/` (not in a subfolder).
...
Read `standards/index.yml` to get the list of available standards and their descriptions.
...
@standards/api/error-handling.md
@standards/global/naming.md
...
Arguments can be:
- **Folder name** — `api` → inject all `.md` files in `standards/api/`
- **Folder/file** — `api/response-format` → inject `standards/api/response-format.md`
- **Root folder** — `root` → inject all `.md` files directly in `standards/` (not in subfolders)
- **Root file** — `root/naming` → inject `standards/naming.md`
