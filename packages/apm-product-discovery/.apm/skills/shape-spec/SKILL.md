---
description: "Process-driven workflow for gathering context and structuring technical plans for significant features or changes."
allowed-tools:
  - bash
  - read
  - grep
...
Check if `product/` exists and contains files.

If it exists, read key files (like `mission.md`, `roadmap.md`, `tech-stack.md`) and use AskUserQuestion:

```
I found product context in product/. Should this feature align with any specific product goals or constraints?
...
Read `standards/index.yml` to identify relevant standards based on the feature being built.
...
**Note:** If `specs/` doesn't exist, create it when saving the spec folder.
...
```
specs/{YYYY-MM-DD-HHMM-feature-slug}/
